...
const LoginPage: React.FC<LoginPageProps> = ({ onLoginSuccess, onBackToLanding }) => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (username.toLowerCase() === 'superadmin') {
      if (password === 'superadmin123') {
        ...
        onLoginSuccess(superadminUser, 'super_admin');
        setLoading(false);
        return;
      } else {
  ...
  const isSuperadminLogin = username.toLowerCase() === 'superadmin';
    ...
    <div className="min-h-screen w-full flex flex-col items-center justify-center p-4 anbk-bg-pattern font-sans">
        <div className="w-full max-w-sm">
            <div className="text-center mb-8">
                ...
                <h1 className="text-2xl font-black text-slate-800 uppercase tracking-tight">Emes CBT Portal</h1>
            ...
            <div className="bg-white/80 backdrop-blur-sm rounded-xl shadow-2xl border border-white/50">
                <div className="p-8">
                    <form onSubmit={handleLogin} className="space-y-5">
                        ...
                        <div className={`transition-all duration-300 ${isSuperadminLogin ? 'max-h-0 opacity-0 overflow-hidden' : 'max-h-40 opacity-100'}`}>
            ...
            <button onClick={onBackToLanding} className="mt-8 text-xs font-bold text-slate-500 uppercase flex items-center gap-2 hover:text-slate-800 mx-auto">
               <ArrowLeft size={14}/> Kembali ke Beranda
             </button>
            <p className="text-center text-xs text-emerald-900/40 font-bold mt-8">© 2026 Emes EduTech</p>
        </div>
    </div>
  );
};
